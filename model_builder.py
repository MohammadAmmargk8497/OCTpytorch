...
class SqueezeExcite(nn.Module):
    def __init__(self, inp, reduction=4):
        ...
        self.fc = nn.Sequential(
                nn.Linear(inp, _make_divisible(inp // reduction, 8)),
                nn.ReLU(inplace=True),
                nn.Linear(_make_divisible(inp // reduction, 8), inp),
                nn.Hardsigmoid()
        )
...
class MV2Block(nn.Module):
    def __init__(self, inp, oup, stride=1, expansion=8):
        super().__init__()
        self.stride = stride
...
if __name__ == '__main__':
    ...
    vit = mobilevit_xxs(1000)
    out = vit(img)
    print(out.shape)
    print(count_parameters(vit))

    vit = mobilevit_xs(1000)
    out = vit(img)
    print(out.shape)
    print(count_parameters(vit))

    vit = mobilevit_s(1000)
    out = vit(img)
    print(out.shape)
    print(count_parameters(vit))